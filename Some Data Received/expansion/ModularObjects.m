...
"expressInTermsOfGenerators[seedsList, genList, phi] expresses phi in terms of the Weyl invariant polynomials given in genList, with seedsList the fundamental coweights used to generate genList. The first generator in genList is assume to be the constant polynomial. phi can also contain unknown coefficients, which will be fixed to render phi Weyl invariant. These coefficients can be found in the variable solCoeffs."
phiInv::usage = 
...
numberOfTerms=1;
ansatz=c[0][1];
For[i=1,i<= index,i++,monomsRaw=Flatten[Permutations[#]&/@IntegerPartitions[i,{rank},Range[0,i]],1];numberOfTerms=numberOfTerms+Length[monomsRaw];monoms=Product[P[k]^#[[rank+1-k]],{k,1,rank}]&/@monomsRaw;ansatz=ansatz+sumList[monoms,c[i]]];
ansatz];
...
vectorToMonis[x_]:=Times@@(P[#]^x[[#]]&/@Range[Length[x]]);

expressInTermsOfGenerators[seedsList_,genList_,poly_]:=Module[{},
nGens=Length[genList]-1;
XList=X[#]&/@Range[nGens];
subPgen=P[#]-> genList[[#+1]]&/@Range[0,nGens];
seedsListT=Transpose[seedsList];
polyToMonis=List@@Expand[poly];
moniToVector[x_]:=Exponent[x,X[#]]&/@Range[Length[seedsList]];
monisToVectors=moniToVector[#]&/@polyToMonis;
decompositions=LinearSolve[seedsListT,#]&/@monisToVectors;
allEntriesPositiveOrZero[list_]:= Module[{},
And@@(#>= 0&/@list)];
linGenListVectors=Select[decompositions,allEntriesPositiveOrZero[#]&];
nOfg=Length[linGenListVectors];
linGenListMonis=vectorToMonis[#]&/@linGenListVectors;
ansatz=Plus@@(c[#]linGenListMonis[[#]]&/@Range[nOfg]);
fracPow=If[(fractionalGens=Denominator[#]&/@Select[Flatten[Exponent[genList,X[#]]&/@Range[rank]],0<#<1&])=={},1,LCM@@(fractionalGens)];XtoY=X[#]-> Y[#]^fracPow&/@Range[rank];YList=Y[#]&/@Range[rank];PList=P[#]&/@Range[0,nGens];genListY=genList/.XtoY//Expand//PowerExpand;exprY=expr/.XtoY//Expand//PowerExpand;MaxYinExpr=Exponent[exprY,Y[#]]&/@Range[rank];subPgenY=P[#]-> genListY[[#+1]]&/@Range[0,nGens];
ansatzY=PowerExpand[ansatz/.subPgenY];
polyY=PowerExpand[poly/.XtoY];
solCoeffs=SolveAlways[ansatzY== polyY,YList][[1]];
ansatz/.solCoeffs]
phiInv[seedsList_,genList_,phi_]:=Module[{i},
	phiCL=CoefficientList[phi,q];
	lenQ=Length[phiCL];
	phiInvCL=expressInTermsOfGenerators[seedsList,genList,#]&/@phiCL;
	phiInvCLq=phiInvCL[[#]]q^(#-1)&/@Range[lenQ];
	Plus@@phiInvCLq];

phiInvNorm[seedsList_, genList_,phi_]:=Module[{},
phiI=phiInv[seedsList, genList,phi];norm=If[Or[(normT=Coefficient[phiI/.q-> 0,P[rank]])===0,Not[NumberQ[normT]]],1,normT];Cancel[phiI/norm]];
...
getPhi[weight_,index_]:=Module[{i,j,w,l},weightG[mults_]:=Sum[mults[[i]]wiGens[[i]][[1]],{i,1,rank+1}];Jaclist={};NofT=0;iGs=0;iGauge=index;w=weight;(* distribution of Gs index between A and B *)distInGs={#,iGs-#}&/@Range[0,iGs];(* distribution of gauge index between rankB31+1 generators, all of index 1 would be the following simple command*)(* distInGauge=Flatten[Permutations[#]&/@IntegerPartitions[iGauge,{rank+1},Range[0,iGauge]],1];*)
(* With total index say 10, and indices 1,1,1,2, we first see how to partition 10 into 1's and 2's. Next we distribute the number of 1's among three phis with index 1, and the number of 2's among 1 phi of index 2.*)distInGauge=distributeIndex[iGauge];phis=Flatten[Outer[Join,distInGs,distInGauge,1],1];Phis=(A^#[[1]] B^#[[2]] Product[Phi[i]^#[[i+2]],{i,1,rank+1}])&/@phis;weightPhis=(weightG[Take[#,-(rank+1)]]-2#[[1]])&/@phis;weightEisenstein=(w-weightPhis);posElligiblePhis=Position[weightEisenstein,_?(#>= 0&),1];elligiblePhis=Flatten[Phis[[#]]&/@posElligiblePhis];weightEisenstein=Flatten[weightEisenstein[[#]]&/@posElligiblePhis];distwEis=IntegerPartitions[#,All,{4,6}]&/@weightEisenstein;For[i=1,i<= Length[elligiblePhis],i++,
